...
import image23 from "./_assets/pg/inicis2.png";
...
- 관리자 콘솔 화면에서 \[결제 연동] → \[연동 관리] → \[채널 관리] 메뉴에서 연동하고자 하는 결제대행사 및 결제 모듈을 선택하여 채널을 추가합니다.
  ...
  <Details id="channel-config-tosspayments">
    ...
    <VersionGate>
      ...
      <IntegrationTosspayments slot="v2" />
    </VersionGate>
  </Details>

  <VersionGate default="v1">
    <Details id="channel-config-settle">
      <p slot="summary">헥토파이낸셜</p>
  ...
  <Details id="channel-config-paypal">
    ...
    <VersionGate>
      <Tabs slot="v1">
        <Tab title="페이팔 일반결제(SPB) / 정기결제(RT)">
          <IntegrationPaypalV2 />
        </Tab>

        <Tab title="페이팔 일반결제(Express Checkout)">
